...
parser.add_argument("--dataset_name", type=str, default="ukiyoe2photogauss",
                    help="name of the dataset")  ## ../input/facades-dataset
...
opt.epoch = 0
opt.n_epochs = 5
print(opt) # @Namespace(b1=0.5, b2=0.999, batch_size=1, channels=3, checkpoint_interval=-1, dataset_name='facades', decay_epoch=3, epoch=0, img_height=256, img_width=256, lambda_cyc=10.0, lambda_id=5.0, lr=0.0003, n_cpu=2, n_epochs=5, n_residual_blocks=9, sample_interval=100)


## 创建文件夹
os.makedirs("images/%s" % opt.dataset_name, exist_ok=True) # ?未解析地址 images/facades
os.makedirs("save/%s" % opt.dataset_name, exist_ok=True) # ?未解析地址 save/facades
...
# @在models文件里
## 创建生成器，判别器对象
G_AB = GeneratorResNet(input_shape, opt.n_residual_blocks) # @9个残差块儿
G_BA = GeneratorResNet(input_shape, opt.n_residual_blocks) # @9个残差块儿
...
criterion_GAN = torch.nn.MSELoss() # @均方误差
criterion_cycle = torch.nn.L1Loss() # @绝对值
...
if opt.epoch != 0:
    # 载入训练到第n轮的预训练模型
    G_AB.load_state_dict(torch.load("saved/%s/G_AB_%d.pth" % (opt.dataset_name, opt.epoch-1)))
    G_BA.load_state_dict(torch.load("saved/%s/G_BA_%d.pth" % (opt.dataset_name, opt.epoch-1)))
    D_A.load_state_dict(torch.load("saved/%s/D_A_%d.pth" % (opt.dataset_name, opt.epoch-1)))
    D_B.load_state_dict(torch.load("saved/%s/D_B_%d.pth" % (opt.dataset_name, opt.epoch-1)))
else:
    # 初始化模型参数
...
dataloader = DataLoader(  ## 改成自己存放文件的目录
    ImageDataset("datasets/ukiyoe2photogauss", transforms_=transforms_, unaligned=True),
...
val_dataloader = DataLoader(
    ImageDataset("datasets/ukiyoe2photogauss", transforms_=transforms_, unaligned=True, mode="test"),  ## "./datasets/facades"
    batch_size=5,
    # shuffle=True,
    shuffle=False,
    num_workers=1,
...
def train():
    ...
    # print("save my model finished !!")
       ## 每间隔几个epoch保存一次模型
        # print(opt.checkpoint_interval, epoch)
        # if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        #     # Save model checkpoints
        #     torch.save(G_AB.state_dict(), "saved/%s/G_AB_%d.pth" % (opt.dataset_name, epoch))
        #     torch.save(G_BA.state_dict(), "saved/%s/G_BA_%d.pth" % (opt.dataset_name, epoch))
        #     torch.save(D_A.state_dict(), "saved/%s/D_A_%d.pth" % (opt.dataset_name, epoch))
        #     torch.save(D_B.state_dict(), "saved/%s/D_B_%d.pth" % (opt.dataset_name, epoch))
        torch.save(G_AB.state_dict(), "saved/%s/G_AB_%d.pth" % (opt.dataset_name, epoch))
        torch.save(G_BA.state_dict(), "saved/%s/G_BA_%d.pth" % (opt.dataset_name, epoch))
        torch.save(D_A.state_dict(), "saved/%s/D_A_%d.pth" % (opt.dataset_name, epoch))
        torch.save(D_B.state_dict(), "saved/%s/D_B_%d.pth" % (opt.dataset_name, epoch))
